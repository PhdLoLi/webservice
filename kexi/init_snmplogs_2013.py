...
class Index:
	...
	def POST(self):
		...
		if name in buffer:
			...
			os.system('impala-shell.sh -i master -d snmp -q " drop table if exists %s; create table %s(time bigint,oid string,ip string, value string); create table %spar like %s stored as parquetfile;"' %(name,name,name,name))
