...
class Index:
	...
	def POST(self):
		...
		#	os.system('impala-shell.sh -i master -d %s -q " drop table if exists %stmp; create table %stmp(time bigint,oid string,ip string, value string); drop table if exists %s; create table %s like %stmp stored as parquetfile;"' %(dbname,name,name,name,name,name))
			os.system('impala-shell.sh -i master -d %s -q "create table %stmp(time bigint,ip string, id string, oids string) row format delimited fields terminated by \'|\'; create table %s like %stmp stored as parquetfile;"' %(dbname,name,name,name))

			conn = MySQLdb.connect(host='localhost',user='root',passwd='881230')
			curs = conn.cursor()
			conn.select_db('%s'%dbname)
			curs.execute("create table %s(time bigint,ip VARCHAR(100), id VARCHAR(100), oids TEXT);"%name)
