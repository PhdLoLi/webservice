...
#		print l_list
		for l in l_list:
			...
			if records!=[]:
				...
				#	print length
					print buffer[key]
					os.system('impala-shell.sh -i master -d snmp -q "insert into dat partition(router=\'%s\', port=%s) values %s"'%(router,port,buffer[key]))
					count[key] = 0 
					buffer[key] = ""
				else:
					buffer[key] = "," + buffer[key] + i_list
					print "That's is too strange! countnow: " + count[key]
					print buffer[key]
					os.system('impala-shell.sh -i master -d snmp -q "insert into dat partition(router=\'%s\', port=%s) values %s"'%(router,port,buffer[key]))
					count[key] = 0 
					buffer[key] = ""
